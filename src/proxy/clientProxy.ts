...
class ClientProxy {
  private _getFuncStreamWay(func: any): { requestStream: boolean; responseStream: boolean } {
  ...
  private _handlerError(err: any, basicMeta: Record<string, unknown>) {
    const newError = new Error() as { name: string; code: string; message: string; stack: string };
  ...
  _proxy(
    ...
    const basicMeta: Record<string, unknown> = { hostname: os.hostname(), appName };

    const target = Object.entries(prototype).reduce((target: any, [name, func]) => {
